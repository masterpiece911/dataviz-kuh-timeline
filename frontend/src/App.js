...
function App() {
    ...
    return ( <
        ...
        onChange = {
            (event) => kaiserClicked(event.target.value) }
        autoWidth >
        {
            kaiser.map((item) => {
                return ( <
        ...
        onClick = {
            () => setGraphData(data) }
        style = {
            { float: 'right', marginRight: 40 } } > IMPORT EXT.DATA <
        /Button> <
        /Grid> <
        /Grid> <
        ...
        /Box> <
        /Typography> <
        FlexibleWidthXYPlot height = { height - 200 }
        yDomain = {
            [minimum(), maximum()] }
        xDomain = { range() }
        ...
        /> <
        stop offset = {
            ((maximum() + 0.5) / (maximum() - minimum()) * 100) + "%" }
        stopColor = "#2699FB"
        stopOpacity = { 0.0 }
        /> <
        /linearGradient> <
        ...
        <
        HorizontalRectSeries data = {
            [{ x0: 1415, x: 1780, y0: -0.5, y: minimum() }] }
        fill = { '#ffffff' }
        stroke = { '#ffffff' }
        />

        {
            kaiserData.map((value) => {
                if (value.id === selectedKaiserID) {
                    return ( <
                        HorizontalRectSeries key = { value.id }
                        data = {
                            [value] }
                        fill = { '#2699FB' }
                        stroke = { '#2699FB' }
                        onSeriesClick = {
                            () => kaiserClicked(value.id) }
                        />
                    )
                }
                return ( <
        ...
        <
        LabelSeries animation data = {
            kaiserData.map((k) => {
                return ({ x: (k.x + k.x0) / 2, y: (k.y + k.y0) / 2 - 1, label: k.name })
            })
        }
        style = {
